...
import { loadRaydium, liquiditySlippagePercent, programIdsForCluster, tokenMetaMint } from '../../lib/raydium'
...
export function PoolPanel() {
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      if (!configsQuery.data?.length) throw new Error('No CPMM fee configs from API')
      const raydium = await loadRaydium({ connection, cluster, wallet })
      ...
      if (mintAAmount.isZero() || mintBAmount.isZero()) throw new Error('Initial amounts must be > 0')
  ...
  const addMutation = useMutation({
    mutationFn: async () => {
      const raydium = await loadRaydium({ connection, cluster, wallet })
      await raydium.account.fetchWalletTokenAccounts({ forceUpdate: true })
      const { poolInfo, poolKeys, rpcData } = await raydium.cpmm.getPoolInfoFromRpc(poolId.trim())
  ...
  const withdrawMutation = useMutation({
    mutationFn: async () => {
      const raydium = await loadRaydium({ connection, cluster, wallet })
      await raydium.account.fetchWalletTokenAccounts({ forceUpdate: true })
      const { poolInfo, poolKeys } = await raydium.cpmm.getPoolInfoFromRpc(poolId.trim())
  ...
  const busy =
    createMutation.isPending || addMutation.isPending || withdrawMutation.isPending
    ...
    <div className="mx-auto max-w-2xl space-y-8">
      <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
        ...
        <p className="mb-4 text-xs text-slate-500">
          Fee tier: {configsQuery.data?.[0] ? `${configsQuery.data[0].tradeFeeRate} bps trade` : '…'}
        </p>
        ...
        {createMutation.error && (
          <p className="mt-2 text-sm text-rose-400">{(createMutation.error as Error).message}</p>
        )}
      </section>
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
        ...
        <div className="mt-4 space-y-3">
          ...
          {addMutation.error && (
            <p className="text-sm text-rose-400">{(addMutation.error as Error).message}</p>
          )}
        </div>
        ...
        <div className="mt-6 space-y-3 border-t border-slate-800 pt-6">
          ...
          {withdrawMutation.error && (
            <p className="text-sm text-rose-400">{(withdrawMutation.error as Error).message}</p>
          )}
        </div>
      </section>
    </div>
